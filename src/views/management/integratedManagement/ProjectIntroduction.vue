  ...
  <div>
    <div class="navigationBar">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        <el-breadcrumb-item>工程介绍</el-breadcrumb-item>
      </el-breadcrumb>
        ...
        <el-main>
          ...
          <div class="eTable">
            <el-table :data="postList" border style="width: 100%" stripe id="out-table" :height="690">
              ...
              <el-table-column prop="preview" label="工程预览图" width="150">
              ...
              <el-table-column prop="projectIntroduce" label="工程介绍" width="380" :show-overflow-tooltip='true'>
              </el-table-column>
           ...
           <el-dialog
            ...
            <span>
              ...
              >
                ...
                <el-form-item label="工程介绍" prop="projectIntroduce">
                  <div class="lookText">
                    <el-input type="textarea" v-model="addForm.projectIntroduce" clearable minlength="940" maxlength="960" show-word-limit></el-input>
                  </div>
                </el-form-item>
            ...
            <span slot="footer" class="dialog-footer">
              ...
              <el-button type="primary" @click="addEngineeringForm()">确 定</el-button>
            </span>
          </el-dialog>
           <!-- 编辑产品 -->
           <el-dialog
            ...
            <span>
              ...
              >
                ...
                <el-form-item label="工程介绍" prop="projectIntroduce">
                  <div class="lookText">
                      <el-input type="textarea" v-model="editForm.projectIntroduce" clearable minlength="940" maxlength="960" show-word-limit></el-input>
                  </div>
                </el-form-item>
                <el-form-item label="排序" prop="orderId">
...
import { addEngineeringInfo,getEngineeringInfo,deleteEngineeringInfo,editEngineeringInfo} from '@/api/projectIntroduction'
export default {
  ...
  methods: {
    ...
    handleCommand(command){
     ...
     resetForm(){
      ...
      this.currentPage = 1
      this.queryInfo=''
      this.showEngineering(1,10)
    },
     //搜索
     searchForm(){
      this.loading = true
      this.currentPage = 1
      this.showEngineering(this.currentPage,10,this.queryInfo)
    },
    resetClear(){
      this.loading=true
      this.currentPage=1 
      this.showEngineering(this.currentPage,10)
    },
    //增加工程
    addContent() {
    ...
    addEngineeringForm(){
      ...
      this.$refs.addFormRef.validate((valid) => {
          if (valid) {
            addEngineeringInfo(this.addForm).then((res)=>{
              if(res.data.code !== 200) return toast('添加工程失败！','error')
              //获取第一页数据
              this.dialogVisible=false
              this.queryInfo=''
              toast('添加工程成功','success')
              this.loading = true
              this.showEngineering(this.currentPage,10)
            })
          } else {
    ...
    //查询初始化数据
    showEngineering(pageNum,pageSize,name=''){
      getEngineeringInfo(pageNum,pageSize,name).then(res=>{
        ...
        if(res.data.result.list.length == 0 && res.data.result.total !== 0){
          this.showEngineering(this.currentPage-1,10)
          this.currentPage--
        }
      }).finally(()=>{
        this.loading=false
    ...
    handleCurrentChange(val){
      ...
      this.loading = true
      this.showEngineering(val,10,this.queryInfo)
    },
    openEditForm(row){
    ...
    confirm(id) {
      this.loading = true
      deleteEngineeringInfo(id).then((res)=>{
    ...
    editCommit(){
      ...
      this.$refs.editFormRef.validate((valid) => {
          if (valid) {
            editEngineeringInfo(this.editForm).then((res)=>{
    ...
    return {
      ...
      addFormRules: {
        ...
        projectIntroduce: [
          ...
          { required: true, message: "请输入工程介绍", trigger: "blur" },
        ],
        orderId: [
